export const lastest_ver = "1.21"

export const addon = [
    {
        title: "MultiAddon (หลายแอดออน)",
        description: "(ไม่มีในช่อง) แต่อยากเอามาใส่",
        imgSrc: 'pack.png',
        readId: "multiaddon",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Level (ระบบเลเวล)",
        description: "เพิ่ม  Structure 'xp' เพื่อโหลดคอมมานเลเวล",
        longDes: '...',
        imgSrc: 'level.jpg',
        readId: "level",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Roblox Chat! (แชทแบบ Roblox)",
        description: "แอดออนนี้จะเพิ่มแชทบนป้ายชื่อของผู้เล่นโดยแอดมินจะมีสิทธิ์ตั้งค่าได้!",
        imgSrc: 'rbchat.jpg',
        readId: "rbchat",
        pageHref: "#",
        isStable: false
    },
    {
        title: "Wanted! (ล่าค่าหัว)",
        description: "แอดออนนี้เป็นระบบล่าค่าหัวพื้นฐานผู้ดูแลมีสิทธิ์ตั้งค่าแอดออนได้",
        imgSrc: 'wanted.jpg',
        readId: "wanted",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Ban/Unban! (การ แบนและปลดแบน)",
        description: "แอดออนนี้เป็นแอดออนที่เพิ่มสิทธิ์ให้ผู้ดูแลเซิร์ฟวเอร์แบนหรือปลดแบนผู้เล่นในเซิร์ฟเวอร์",
        imgSrc: 'ban.jpg',
        readId: "ban",
        pageHref: "#",
        isStable: false
    },
    {
        title: "VoteKick (โหวตเตะผู้เล่น)",
        description: "แอดออนนี้เพิ่มความมีส่วนร่วมให้กับผู้เล่นในเซิร์ฟเวอร์ของคุณ ทำให้พวกเขาสามารถโหวตคนนิสัยแย่ ๆ ในเซิร์ฟเวอร์ออกไปได้!",
        imgSrc: 'voteKick.jpg',
        readId: "voteKick",
        pageHref: "#",
        isStable: true
    },
    {
        title: "UnBreak (ระบบกันพัง)",
        description: "แอดออนนี้เราแค่เห็นว่ามันเท่ดีเลยทำมาแจกเป็นระบบกันทุบบล็อก/วางบล็อก (โหมดผจญภัยดีกว่า)",
        imgSrc: 'unbreak.jpg',
        readId: "unbreak",
        pageHref: "#",
        isStable: false
    },
    {
        title: "All Chat (รวมระบบแชท)",
        description: "แอดออนนี้เป็นแอดออนที่ผมรวบรวมระบบแชทที่เคยแจกก่อนหน้าเป็นอันเดียวแล้วมาแจกใหม่!",
        imgSrc: 'allchat.jpg',
        readId: "allchat",
        pageHref: "#",
        isStable: false
    },
    {
        title: "Dash (พุ่งหลาว)",
        description: "แอดออนนี้เพิ่มแท็ก Dash เพื่อนำไปใช้ควบคู่กับคอมมานบล็อกต่าง ๆ",
        imgSrc: 'dash.jpg',
        readId: "dash",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Moon Action (ตรวจสอบการ..)",
        description: "เจ้าตัวนี้จะช่วยเพิ่ม tag/score ให้กับคนที่ทำอะไรบางอย่างเช่น ย่อ กระโดด วิ่ง",
        imgSrc: 'moonaction.jpg',
        readId: "moonaction",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Health NameTag (แสดงเลือด mob)",
        description: "แอดออนนี้จะเปลีย่นชื่อ mob ในเซิร์ฟเวอร์ให้แสดงเลือด รองรับป้ายชื่อด้วยแหละ!",
        imgSrc: 'healthametag.jpg',
        readId: "healthametag",
        pageHref: "#",
        isStable: true
    },
    {
        title: "RoomChat (ห้องแชท)",
        description: "ระบบห้องแชทสำหรับเซิร์ฟเวอร์มินิเกมและอื่น ๆ ระบบนี้จะช่วยได้อย่างมาก",
        imgSrc: 'roomchat.jpg',
        readId: "roomchat",
        pageHref: "#",
        isStable: false
    },
    {
        title: "NearChat (แชทใกล้เคียง)",
        description: "แอดออนนี้เปิดโอกาสให้ผู้เล่นสามารถแชทกับผู้คนบริเวณรอบ ๆ ตัวได้ ตามที่แอดมินเซิร์ฟเวอร์กำหนดไว้",
        imgSrc: 'nearchat.jpg',
        readId: "nearchat",
        pageHref: "#",
        isStable: false
    },
    {
        title: "ChatRank (แชทแรงค์)",
        description: "แอดออนนี้จะทำให้ชื่อของผู้เล่นระหว่างแชทมีแรงค์ขึ้นต้น!",
        imgSrc: 'chatrank.jpg',
        readId: "chatrank",
        pageHref: "#",
        isStable: false
    },
    {
        title: "PoliceCaughtThief (ตำรวจจับโจร)",
        description: "แอดออนนี้เพิ่มระบบตำรวจจับโจรแบบง่าย ๆ แต่มีการตั้งค่าที่เยอะมาก!",
        imgSrc: 'policecaughtthief.jpg',
        readId: "policecaughtthief",
        pageHref: "#",
        isStable: true
    },
    {
        title: "AFK (อยู่หน้าจอหรือไม่)",
        description: "เพิ่ม structure สำหรับตรวจคนที่ afk ในเซิร์ฟเวอร์",
        imgSrc: 'afk.jpg',
        readId: "afk",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Real Time (เวลาจริง)",
        description: "เพิ่ม scoreboard สำหรับดูเวลาในโลกจริง",
        imgSrc: 'realtime.jpg',
        readId: "realtime",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Anti Spam (ป้องกันสแปม)",
        description: "ป้องกันไม่ให้คนฟรัดข้อความในเซิร์ฟเวอร์ (แอดมินต้องค่าข้อความและเวลาได้)",
        imgSrc: 'antispam.jpg',
        readId: "antispam",
        pageHref: "#",
        isStable: false
    },
    {
        title: "Leader Board (กระดานผู้นำ)",
        description: "กระดานผู้นำตั้งค่าได้สำหรับแอดมิน~",
        imgSrc: 'leaderboard.jpg',
        readId: "leaderboard",
        pageHref: "#",
        isStable: true
    },
    {
        title: "Transfer Money (ระบบโอนเงิน)",
        description: "แอดออนโอนเงินให้ผู้เล่นอื่น ๆ ในเซิร์ฟเวอร์",
        imgSrc: 'transfermoney.jpg',
        readId: "transfermoney",
        pageHref: "#",
        isStable: true
    },
    {
        title: "InvSee (ส่องช่องเก็บของ)",
        description: "ส่องช่องเก็บของผู้เล่นในเซิร์ฟเวอร์ (แอดมินเท่านั้น)",
        imgSrc: 'invsee.jpg',
        readId: "invsee",
        pageHref: "#",
        isStable: false
    },
    {
        title: "TeleportCompass (เข็มทิศวาร์ปหนี)",
        description: "แอดออนนี้จะทำให้เข็มทิศวาร์ปไปหาผู้เล่นในเซิร์ฟเวอร์ของคุณได้",
        imgSrc: 'teleportcompass.jpg',
        readId: "teleportcompass",
        pageHref: "#",
        isStable: true
    },
    {
        title: "ColorName (เปลี่ยนสีชื่อ)",
        description: "เปลี่ยนชื่อป้ายชื่อให้กับผู้เล่นในเซิร์ฟเวอร์",
        imgSrc: 'nametag.jpg',
        readId: "nametag",
        pageHref: "#",
        isStable: true
    },
    {
        title: "ServerUI (เมนูผู้เล่น)",
        description: "เพิ่มให้นาฬิกามีเมนูเปลี่ยนภาษาได้ด้วยนะ",
        imgSrc: 'serverui.jpg',
        readId: "serverui",
        pageHref: "#",
        isStable: true
    }
]

export const find_me = [
    // http://127.0.0.1:5500/download/?lastest=antispam
    // http://127.0.0.1:5500/download/?ver1.20.40=antispam
    // https://mcbe.vercel.app/download/?lastest=allchat
    { url: "multiaddon", link: "https://github.com/aitji/multiAddon/releases/download/1.1.5/multiAddon.mcpack" },
    { url: "afk", link: "https://github.com/aitji/aitji.addon/raw/main/Other/AFK/Download.mcpack" },
    { url: "allchat", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/All%20Chat/Download.mcpack" },
    { url: "antispam", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/AntiSpam/Download.mcpack" },
    { url: "blood_addon", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Blood/Download%20Addon.mcpack" },
    { url: "blood_res", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Blood/Download%20Resource.mcpack" },
    { url: "dash", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Dash/Download.mcpack" },
    { url: "healthametag", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Health%20NameTag/Download.mcpack" },
    { url: "invSee", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/InvSee/Download.mcpack" },
    { url: "leaderboard", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Leaderboard/Download.mcpack" },
    { url: "moonaction", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Moon%20Action/Download.mcpack" },
    { url: "nametag", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/NameTag/Download.mcpack" },
    { url: "nearchat", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/NearChat/Download.mcpack" },
    { url: "policecaughtthief", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Police%20Caught%20Thief/Download.mcpack" },
    { url: "rankchat", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/RankChat/Download.mcpack" },
    { url: "realtime", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Real-Time/Download.mcpack" },
    { url: "roomchat", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/Room%20Chat/Download.mcpack" },
    { url: "serverui", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Server%20UI/Download.mcpack" },
    { url: "teleportcompass", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Teleport%20Compass/Download.mcpack" },
    { url: "transfermoney", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Transfer%20Money/Download.mcpack" },
    { url: "unbreak", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/UnBreak/Download.mcpack" },
    { url: "votekick", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/VoteKick/Download.mcpack" },
    { url: "ban", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/ban/Download.mcpack" },
    { url: "wanted", link: "https://github.com/aitji/aitji.addon/raw/main/Stable/Wanted/Download.mcpack" },
    { url: "rbchat", link: "https://github.com/aitji/aitji.addon/raw/main/_ver_/rbchat/Download.mcpack" },
    { url: "level", link: "https://github.com/aitji/aitji.addon/raw/main/Other/Level/Download.mcpack" },
]